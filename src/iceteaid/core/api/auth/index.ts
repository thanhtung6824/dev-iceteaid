...
export class AuthApi extends BaseApi {
    ...
    public isLogin(): ReturnType<boolean> {
        const payload = queryBuilder(RequestType.IS_LOGIN, {});
        return this.transporter.post(this.iframe, payload);
    }

    public async loginWithLink(email: string, showPopup = true): Promise<ReturnType<boolean>> {
        if (showPopup) {
            (this.iframe as any).openIframe();
        }
        const payload = queryBuilder(RequestType.LOGIN_WITH_LINK, { email });
        const response = await this.transporter.post(this.iframe, payload);
        (this.iframe as any).closeIframe();
        return response;
    }
}